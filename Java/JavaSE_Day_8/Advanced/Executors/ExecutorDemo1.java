...
public class ExecutorDemo1 {
	public static void main(String args[]) {
		...
		ExecutorService exec = Executors.newCachedThreadPool(); // (Executors.newCachedThreadPool();) Implementation class of ExecutorService

		for (int i = 0; i < 3; i++) {
			exec.execute(new myapp()); // similar to start method here myapp() is target
		}
		 ...
		 * current thread ( e.g. main thread ) will continue to run all tasks submitted
