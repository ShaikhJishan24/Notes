...
public class Main {
     public static void main(String[] args) {
          ...
          Root<Employee> root = criteria1.from(Employee.class);  // used for filteration to specify which field do you want
          ...
          criteria2.multiselect(cb.count(root));  // works on multiple record 
          ...
          criteria3.where(cb.equal(root.get("salary"), 25000.00)); // where equal condition 
          ...
          criteria4.where(cb.greaterThan(root.get("salary"), 25000.00)); // where greaterthan condition
          ...
          criteria5.where(cb.like(root.get("name"), "p%")); // like clause
          ...
          criteria6.where(cb.between(root.get("salary"), 30000, 50000)); // between 
          ...
          criteria7.where(cb.and(cb.greaterThan(root.get("salary"), 20000)), cb.equal(root.get("desig"), "manager")); // and clause
          ...
          criteria8.where(cb.or(cb.equal(root.get("desig"), "accountant"), cb.equal(root.get("desig"), "clerk"))); // In opeartor
          ...
          criteria10.orderBy(cb.asc(root.get("salary"))); // orderBy
          ...
          criteria11.orderBy(cb.desc(root.get("salary")));  // descending
          ...
          criteria12.multiselect(cb.avg(root.get("salary")));  // average for multiselect 
          ...
          criteria13.multiselect(cb.min(root.get("salary")));  // min
          ...
          criteria14.multiselect(cb.max(root.get("salary")));  // max
          ...
          criteria16.select(root.get("name"));   // to select any field
          criteria16.where(cb.equal(root.get("salary"), 25000.00));
          q = session.createQuery(criteria16);
          results = q.getResultList();
          System.out.println(results);

          // Note - multiselect used when we used Aggregate function
     }
}
